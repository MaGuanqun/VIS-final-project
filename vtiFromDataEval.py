...
if __name__ == "__main__":
    data_path = "./data_old_refactored"
    data_idx = 1053
    output_name = "./test"
    model_class = inplaceCNNTwoLevel
    model_params = "./model_firstTwo.th"
    ...
    model.eval()
    labels_mine = model(sf_rect)

    pred = labels_mine[0].argmax(dim=0)
    cps = (pred == 1)
    norms = (pred == 0)
    pred[cps] = labels_mine[1][:,cps].argmax(dim=0)
    pred[norms] = 3
